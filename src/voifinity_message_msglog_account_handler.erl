-module(voifinity_message_msglog_account_handler).
-export([init/2
	    ,handle_req/3
        ,json_creation/3
        ]).
init(Req0, State) ->
  ...
  {ok, Req0, State}.
handle_req(<<"GET">>,[<<"message_log">>,<<"account">>,AccountId],Req) ->
  case Data= mnesia:dirty_index_read(storemessage,AccountId,account_id) of
    [] ->
...
json_creation([H|T],Accumulator,Req) ->
  ...
  Out=[{<<"from">>,From},{<<"To">>,To},{<<"datetime">>,Datetime},{<<"message">>,Payload}],
  json_creation(T,[Out|Accumulator],Req).
